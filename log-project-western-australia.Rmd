...
ggplot()+
  ...
  geom_polygon(data=hulldat, aes(NMDS1, NMDS2, fill=treatment, group=treatment), alpha=0.3)+scale_fill_manual(values=c("#63A088","#56638A"))+
  geom_point(data=mds_scores, aes(NMDS1, NMDS2, shape=block, col=treatment), size=4)+ scale_shape_manual(values = c(4,15,16,17,11,9,8)) +scale_colour_manual(values=c("#63A088","#56638A"))+
  geom_text_repel(data=species.scores, aes(NMDS1, NMDS2, label=species), alpha=0.5, size=4)
```
</center>
