...
species.scores.t2$species<-rownames(species.scores) 
...
library(ggpubr)
```
nutrient analysis (nutrient composition ~ log vs open; abundance ~ nutrient composition , diversity ~
nutrient composition, composition ~ nutrient composition)
...
Nutrient composition between log and open
...
for (element in c(Nu, "organic.carbon")) {
  if (element == "organic.carbon") {
    plot <- ggplot(nutrient, aes(x = init, y = organic.carbon)) +
      ...
      geom_signif(comparisons = list(c("insitu_log", "open")),
...
# look at range of data - what family should i use? 
range(dat_t0_insitu$total)
nutrient_join <- nutrient[,(3:16)]
dat_t0_insitu <- inner_join(dat_t0_insitu, nutrient_join, by = "init")
...
for (element in Nu) {
  ...
  abun.mod <- glmer(formula, data = subset_data, family = poisson, subset = (time == "t2"))
  model_summary <- summary(abun.mod)
...
dat$diversity<-diversity(est, index='shannon') 
nutrient_join <- nutrient[,(3:16)]
dat <- inner_join(dat, nutrient_join, by = "init")
...
hurd.mod.1.t2 <- data.frame(Element = character(), P_Value = numeric(), stringsAsFactors = FALSE)
...
for (element in Nu) {
  ...
  hurd.mod.1.t2 <- rbind(hurd.mod.1.t2, data.frame(Element = element, P_Value = p_value, stringsAsFactors = FALSE))
}
