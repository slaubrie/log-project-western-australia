...
##### Diversity analysis
```{r, message=TRUE, warning=TRUE, include=TRUE, paged.print=FALSE, results='markup', fig.width =8, fig.height=8}
require(vegan)
require(dplyr)
require(tidyr)
require(labdsv)
...
tab_model(Hurd.mod.1, Hurd.mod.2, dv.labels=c("Prob. SDI>0", "Pron. when SDI>0"), title = "Shannon diversity index (SDI): 2020-2022", file = "sdi_openlog_allyear.html")
...
# Open and log environment is significant in explaining the plant diversity in transect levels.
# Lmer was used because the distribution of non-zero data is approximally normal.
Hurd.mod.2 <- lm(diversity ~ init, data=subset(dat_2020, non_zero == 1))
## random effect (1|block) removed since it's close to zero
# summary(Hurd.mod.2)
# anova(Hurd.mod.2) # lmer on non-zero diversity values is significant

tab_model(Hurd.mod.1, Hurd.mod.2, dv.labels=c("Prob. SDI>0", "Pron. when SDI>0"), title = "Shannon diversity index (SDI): 2020", file = "sdi_openlog_2020.html")
...
# Ppen and log environment does determine the probability of non-zero diversity index
# 0 Shannon diversity index =/= zero plants
Hurd.mod.1 <- glm(non_zero ~ init, data = dat_2021, family = binomial)
# Random effect (1|block) removed since it's close to zero.
# summary(Hurd.mod.1) # significant
...
tab_model(Hurd.mod.1, Hurd.mod.2, dv.labels=c("Prob. SDI>0", "Pron. when SDI>0"), title = "Shannon diversity index (SDI): 2021", file = "sdi_openlog_2021.html")
...
tab_model(Hurd.mod.1, Hurd.mod.2, dv.labels=c("Prob. SDI>0", "Pron. when SDI>0"), title = "Shannon diversity index (SDI): 2022", file = "sdi_openlog_2022.html")
...
```{r, message=FALSE, warning=FALSE, include=TRUE, paged.print=FALSE, results=TRUE, fig.width=12, fig.height=12, out.width=c('30','30', '100')}
# subset data where all t0 communities, insitu log and insitu open communities at t1 and t2 are included.
...
((plots$`1+3` + plots$`1+4`+ plots$`1+5`)/(plots$`2+3` + plots$`2+4`+ plots$`2+5`)/(plots$`3+4` + plots$`3+5`+ plots$`4+5`)) + plot_layout(guides ="collect") + plot_annotation(paste0("Stress:",round(ass.rel.t1_NMS$stress,3), " (k = 5)"))
```
...
((plots$`1+3` + plots$`1+4`+ plots$`1+5`)/(plots$`2+3` + plots$`2+4`+ plots$`2+5`)/(plots$`3+4` + plots$`3+5`+ plots$`4+5`)) + plot_layout(guides ="collect") + plot_annotation(paste0("Stress:",round(ass.rel.t012_NMS$stress,3), " (k = 5)"))
