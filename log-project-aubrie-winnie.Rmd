...
trt_Frac<-rda(ass.rel.t0~init+Condition(block)) # run model using standardized data 
# summary(trt_Frac)
...
result.df.t0 <- data.frame(
  ...
  Partitioning_of_variance = c("rda(formula = ass.rel.t0 ~ init + Condition(block)) ", "Total", "Conditioned", "Constrained", "Unconstrained"),
  Inertia  = c(" ", 0.86477, 0.07929, 0.01176, 0.77372),
  Proportion = c("",1.00000, 0.09169, 0.01360, 0.89471),
  Note = c(" ", "adj.r.squared = 0.009696575", "Variance explained by 'block'","Variance explained by 'treatment'", "Unexplained variance"),
  Permutation_test = c("Model signif. = ***", "treatment", "Residual", " ", ""),
  Df = c(" ", 1,215,"", ""),
  Variance = c(" ", 0.01176, 0.77372,"", ""),
  F = c(" ", 3.2671, " ", " ", " "),
  P = c(" ", "0.001 ***", " ", " ", "")
  )
...
ano_t0 <- anosim(assemblies_t0, blocksum$init, distance = "bray", permutations = 9999)
...
# redundancy analysis
trt_Frac<-rda(ass.rel.t1~init+ Condition(block)) # run model using standardized data 
# summary(trt_Frac)
# anova.cca(trt_Frac, step=1000, by="term") ## test for model significance
# anova.cca(trt_Frac)
...
result.df.t1 <- data.frame(
  stringsAsFactors = FALSE,
  Partitioning_of_variance = c("rda(formula = ass.rel.t1 ~ init + Condition(block))", "Total", "Conditioned", "Constrained", "Unconstrained"),
  Inertia  = c(" ", 0.82992, 0.11182, 0.01785, 0.70025),
  Proportion = c("",1.00000, 0.13473, 0.02151, 0.84376),
  Note = c(" ", "adj.r.squared = 0.01150461", "Variance explained by 'block'","Variance explained by 'treatment'", "Unexplained variance"),
  Permutation_test = c("Model signif. = **", "treatment", "Residual", " ", ""),
  Df = c(" ", 1,78,"", ""),
  Variance = c(" ", 0.01785, 0.70025,"", ""),
  F = c(" ", 1.9885, " ", " ", " "),
  P = c(" ", "0.01 **", " ", " ", "")
  )
...
ano_t1 <- anosim(assemblies_t1, blocksum$init, distance = "bray", permutations = 9999)
...
# redundancy analysis
trt_Frac<-rda(ass.rel.t2~init+ Condition(block)) # run model using standardized data 
# summary(trt_Frac)
# anova.cca(trt_Frac, step=1000, by="term") ## test for model significance
# anova.cca(trt_Frac)
# RsquareAdj(trt_Frac)$adj.r.squared #explanatory power
...
result.df.t2 <- data.frame(
  stringsAsFactors = FALSE,
  Partitioning_of_variance = c("rda(formula = ass.rel.t2 ~ init + Condition(block))", "Total", "Conditioned", "Constrained", "Unconstrained"),
  Inertia  = c(" ", 0.81670, 0.10345, 0.02511, 0.68814),
  Proportion = c("",1.00000, 0.12667, 0.03075, 0.84259),
  Note = c(" ", "adj.r.squared = 0.02119015", "Variance explained by 'block'","Variance explained by 'treatment'", "Unexplained variance"),
  Permutation_test = c("Model signif. = **", "treatment", "Residual", " ", ""),
  Df = c(" ", 1,76,"", ""),
  Variance = c(" ", 0.02511, 0.68814,"", ""),
  F = c(" ", 2.7732, " ", " ", " "),
  P = c(" ", "0.002 **", " ", " ", "")
  )
...
ano_t2 <- anosim(assemblies_t2, blocksum$init, distance = "bray", permutations = 9999)
...
result.df.t012 <- data.frame(
  stringsAsFactors = FALSE,
  Partitioning_of_variance = c("Call:rda(X = rda(formula = ass.rel.t012 ~ init + block + Condition(time)) ", "Total", "Conditioned", "Constrained", "Unconstrained"),
  Inertia = c("", 0.56175, 0.08841, 0.19475, 0.27860),
  Proportion = c("",1.0000, 0.1574, 0.3467, 0.4959),
  Note = c("Model signif. = ***", "R2 = 0.250412", "Variance explained by 'year'", "Variance explained by 'Treatment' and 'block'", "Unexplained variance"),
  Permutation_test = c(" ", "treatment", "block", "Residual", " "),
  Df = c(" ",  1,6,32, " "),
  Variance = c(" ", 0.034157, 0.160594,0.278596, " "),
  F = c(" ", 3.9233, 3.0743, " "," "),
  P = c(" ", "0.001 ***", "0.001 ***", " "," ")
  )
...
```{r, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE, results=FALSE, fig.width=12, fig.height=16,echo=FALSE}
...
best.2022 <- glm(total~ N+ P + C + CEC , data = dat_t0_insitu,subset = (time == "t2"), family = poisson)
