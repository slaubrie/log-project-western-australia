...
biplot_2020 <- ggplot(sc_sp_2020, aes(x = RDA1, y = RDA2)) +
  ...
  labs(x = paste0("RDA1 (", perc[1], "%)"), y = paste0("RDA2 (", perc[2], "%)"), title = "RDA biplot [2020]") +
...
biplot_2021 <- ggplot(sc_sp_2021, aes(x = RDA1, y = RDA2)) +
  ...
  geom_text(data = sc_bp_2021, aes(x = sc_bp_2021[,1] -0.08, y = sc_bp_2021[,2] +0.05, label = "open"), 
            color = "black", fontface = 2, size = 4) +
  labs(x = paste0("RDA1 (", perc[1], "%)"), y = paste0("RDA2 (", perc[2], "%)"), title = "RDA biplot [2021]") +
  scale_color_manual(values = c("#63A088", "#56638A"),guide = FALSE) +
  scale_fill_manual(values = c("#63A088", "#56638A"), name = "Treatment") +
...
biplot_2022 <- ggplot(sc_sp_2022, aes(x = RDA1, y = RDA2)) +
  ...
  geom_text(data = sc_bp_2022, aes(x = sc_bp_2022[,1] + 0.15, y = sc_bp_2022[,2] + 0.05, label = "open"), 
            color = "black", fontface = 2, size = 4) +
  labs(x = paste0("RDA1 (", perc[1], "%)"), y = paste0("RDA2 (", perc[2], "%)"), title = "RDA biplot [2022]") +
  scale_color_manual(values = c("#63A088", "#56638A"),guide = FALSE) +
  scale_fill_manual(values = c("#63A088", "#56638A"), name = "Treatment") +
...
(biplot_2020 + biplot_2021 + biplot_2022)+ plot_layout(guides ="collect")
```

Composition dissimilarity *2020 - 2022*
```{r 2020-2022 pooled composition, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE, results=FALSE,echo=FALSE}
# Composition dissimilarity * 2020 - 2022 *
# subset data where all t0 communities, insitu log and insitu open communities at t1 and t2 are included.
mat3 <- mat
...
# Perform ANOVA for each element between blocks
wilcox_block <- data.frame(
  element = character(),
  p_value = numeric(),
  stringsAsFactors = FALSE
)
...
for (element in Nu) {
  result <- aov(get(element) ~ block, data = nutrient)
  wilcox_block <- rbind(wilcox_block, 
                        data.frame(
                          element = element,
...
wilcox_block <- na.omit(wilcox_block)
print(wilcox_block)
...
for (element in c(Nu)) {
  if (element == "C" | element == "N") {
    plot <- ggplot(nutrient, aes(x = init, y = !!rlang::sym(element))) +
      geom_boxplot() + 
      theme_bw() +
      labs(x = NULL, y = ifelse(element == "C", "C (%)", "N (mg/kg)")) +
      labs(x = NULL, y = ifelse(element == "C", "C (%)", "N (mg/kg)")) +
      scale_x_discrete(labels=c('log', 'open')) +
      geom_signif(comparisons = list(c("log", "open")),
                  map_signif_level = TRUE,
                  textsize = 5, vjust = 0.6, tip_length = 0, y_position = ifelse(element=="C", 1.6, 6)) + 
      scale_y_continuous(expand = c(0,0.1), limits= if (element == "N") c(2.5, 6.3)) +
      theme(
    plot.background = element_blank(),
    ...
    plot <- ggplot(nutrient, aes(x = init, y = !!rlang::sym(element))) +
      geom_boxplot() + 
      theme_bw() +
      labs(x = NULL, y = paste(element, ifelse(unit_label != "", paste(" (", unit_label, ")", sep = ""), ""), sep = "")) +
      scale_x_discrete(labels=c('log', 'open')) +
      theme(
    plot.background = element_blank(),
    panel.grid.major = element_blank(),
...
# Access plots_list to see or use the plots
...
[2020-2022 composite data] An averaged model is created by averaging a set of models that have a cumulative sum of Akaike Information Criterion (AIC) weights equal to 0.95. Each element's total weight is the sum of its weights in all models containing it.
...
avg_allyear_full <- rownames_to_column(avg_allyear_full, "Variable")
sum_weights_df <- data.frame(
  Variable = c("P", "pH", "N", "C", "CEC"),
  `Sum of weights` = c(0.72, 0.52, 0.48, 0.47, 0.30)
)

avg_allyear_full <- merge(avg_allyear_full, sum_weights_df, by = "Variable", all.x = TRUE)

avg_allyear_full <- avg_allyear_full %>% 
  slice(1) %>%
  bind_rows(avg_allyear_full %>% slice(-1) %>% arrange(desc(Sum.of.weights)))

knitr::kable(avg_allyear_full, "simple")
...
avg_allyear_full <- rownames_to_column(avg_allyear_full, "Variable")
sum_weights_df <- data.frame(
  Variable = c("P", "pH", "C", "CEC"),
  `Sum of weights` = c(0.309, 0.089, 0.085, 0.018)
)
...
```{r 2020 composition and nutrient, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE, results=FALSE,echo=FALSE}
# Data wrangling
...
ggplot(sc_sp, aes(x = RDA1, y = RDA2)) +
  ...
  geom_point(data = sc_si, aes(color = treatment, shape = factor(block)), size = 5) +
  geom_segment(data = sc_bp, aes(x = 0, y = 0, xend = sc_bp[,1], yend = sc_bp[,2]), 
               arrow = arrow(length = unit(0.3, "cm")), color = "black") +
  geom_text(data = sc_bp, aes(x = sc_bp[,1]*1.2, y = sc_bp[,2]*1.2, label = rownames(sc_bp)), 
            color = "black", fontface = 2, size = 4) +
  labs(x = paste0("RDA1 (", perc[1], "%)"), y = paste0("RDA2 (", perc[2], "%)"), title = "RDA biplot [2020]") +
  scale_color_manual(values = c("#63A088","#56638A"),name = "Treatment") +
...
```{r 2021 composition and nutrient, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE, results=FALSE,echo=FALSE}
# Data wrangling
...
print(en.nutrient) ########## it is basically saying most of the nutrient does not affect the composition of plants.
(nmds.plot.sp + theme(legend.position = "none")) + nmds.plot.nutrient + plot_layout(guides = "collect") + plot_annotation(title = 'Plant composition: 2021')
```

*2022*

```{r 2022 composition and nutrient, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE, results=FALSE,echo=FALSE}
# Data wrangling
# This data set does not include unknown data.
comm <- read.csv("20-22_species_composition_data_no_unk.csv", header = T)
...
trt_tot_2<-rda(ass.rel.t012 ~ N+P+C+pH+CEC + 
...
blk1<-sc_si[sc_si$block == 1, ][chull(sc_si[sc_si$block == 1, c("RDA1", "RDA2")]), ]
blk2<-sc_si[sc_si$block == 2, ][chull(sc_si[sc_si$block == 2, c("RDA1", "RDA2")]), ]
blk3<-sc_si[sc_si$block == 3, ][chull(sc_si[sc_si$block == 3, c("RDA1", "RDA2")]), ]
blk4<-sc_si[sc_si$block == 4, ][chull(sc_si[sc_si$block == 4, c("RDA1", "RDA2")]), ]
blk5<-sc_si[sc_si$block == 5, ][chull(sc_si[sc_si$block == 5, c("RDA1", "RDA2")]), ]
blk6<-sc_si[sc_si$block == 6, ][chull(sc_si[sc_si$block == 6, c("RDA1", "RDA2")]), ]
blk7<-sc_si[sc_si$block == 7, ][chull(sc_si[sc_si$block == 7, c("RDA1", "RDA2")]), ]

hulldat_all<-rbind(blk1,blk2, blk3, blk4, blk5, blk6, blk7)
...
ggplot(sc_sp, aes(x = RDA1, y = RDA2)) +
  ...
  scale_color_manual(values = c("#63A088","#56638A"),guide = FALSE) +
  scale_fill_manual(values = c("#63A088","#56638A"), name = "Block") +
  scale_shape_manual(values = unique(as.numeric(factor(sc_si$block))), name = "Treatment") +
  theme_bw()+
  geom_hline(yintercept = 0, linetype = "dotted") +  
  geom_vline(xintercept = 0, linetype = "dotted") +
  theme(panel.grid.major = element_blank(),  
...
ggplot(sc_sp, aes(x = RDA1, y = RDA2)) +
  ...
  geom_polygon(data = hulldat_all, aes(fill = block, group = block), alpha = 0.3) +
  geom_segment(data = sc_bp, aes(x = 0, y = 0, xend = sc_bp[,1]*1.5, yend = sc_bp[,2]*1.5), 
               arrow = arrow(length = unit(0.3, "cm")), color = "black") +
  geom_text(data = sc_bp, aes(x = sc_bp[,1]*1.7, y = sc_bp[,2]*1.7, label = rownames(sc_bp)), 
            color = "black", fontface = 2, size = 4) +
...
(nmds.plot.sp + theme(legend.position = "none")) + nmds.plot.nutrient + plot_layout(guides = "collect") + plot_annotation(title = 'Plant composition: 2020-2022')
```
